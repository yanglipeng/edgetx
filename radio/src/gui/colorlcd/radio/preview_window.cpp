    ...
    Window(window, rect)
{
  ...
  // White background for QM
  etx_solid_bg(qm->getLvObj(), COLOR_THEME_PRIMARY2_INDEX);

  // Dark top bar with logo
  auto qmTopBar = new Window(qm, {0, 0, QM_W, EdgeTxStyles::UI_ELEMENT_HEIGHT});
  etx_solid_bg(qmTopBar->getLvObj(), COLOR_THEME_PRIMARY3_INDEX);
  auto mask = getBuiltinIcon(ICON_TOP_LOGO);
  new StaticIcon(qmTopBar, (QM_W - mask->width) / 2, 0, ICON_TOP_LOGO, COLOR_THEME_PRIMARY2_INDEX);

  // Button 1: Model Select with PRIMARY3 accent (focused state simulated)
  ...
  etx_solid_bg(qmb->getLvObj(), COLOR_THEME_PRIMARY3_INDEX, LV_PART_MAIN);
  new StaticIcon(qmb, (QuickMenuGroup::QM_BUTTON_WIDTH - QuickMenuGroup::QM_ICON_SIZE) / 2, PAD_SMALL,
                  ICON_MODEL_SELECT, COLOR_THEME_PRIMARY2_INDEX);
  new StaticText(qmb, {0, QuickMenuGroup::QM_ICON_SIZE + PAD_TINY * 2, QuickMenuGroup::QM_BUTTON_WIDTH - 1, 0},
                  STR_QM_MANAGE_MODELS, COLOR_THEME_PRIMARY2_INDEX, CENTERED | FONT(XS));

  // Button 2: Model with SECONDARY1 accent (unfocused state)
  ...
  new StaticIcon(qmb, (QuickMenuGroup::QM_BUTTON_WIDTH - QuickMenuGroup::QM_ICON_SIZE) / 2, PAD_SMALL,
                  ICON_MODEL, COLOR_THEME_SECONDARY1_INDEX);
  new StaticText(qmb, {0, QuickMenuGroup::QM_ICON_SIZE + PAD_TINY * 2, QuickMenuGroup::QM_BUTTON_WIDTH - 1, 0},
