...
static Vs1053bRecorder* s_active = nullptr;
...
FRESULT Vs1053bRecorder::start(const char* path, uint32_t expectedSeconds)
{
  ensureMutex();
...
bool Vs1053bRecorder::tickLocked()
{
  if (!recording) return false;

  uint32_t n = vs1053b_read_recording_data(pcm, PCM_MAX);
  if (n == 0) return false;
...
void Vs1053bRecorder::audioTick()
{
...
FRESULT Vs1053bRecorder::trimSilence(const char* path)
{
  // Simple threshold: PCM values below this are treated as silence.
  // Tuned for VS1053B nominal gain (AICTRL = 128).
  static constexpr int16_t SILENCE_THRESHOLD = 120;
  ...
  for (uint32_t base = 0; base < total; base += CHUNK) {
    ...
    for (uint32_t i = 0; i < got; i++) {
      const int16_t abs = buf[i] < 0 ? -buf[i] : buf[i];
  ...
  if (firstActive == UINT32_MAX) { f_close(&f); return FR_OK; }
